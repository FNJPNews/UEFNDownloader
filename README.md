...
# Diagnostics that may contain private IDs, URLs, tokens, or keys
module_key_v4.json
content_v2_cooked_content_package.json
content_v4_client.json
cooked_content_package.json
content_service_probe.json
artifact_candidates.json
log_cooked_url_candidates.json
...
Use this tool only in ways that comply with:

- Epic Games Terms of Service
- Epic Games Fan Content Policy
- Fortnite / UEFN rules and licenses
- Applicable copyright law
- Any permissions granted by the island or asset owner

Do not use this tool to redistribute copyrighted Fortnite / UEFN assets, bypass access controls, or access content you are not authorized to access.

## Development Notes

The repository should preferably publish only the maintained CLI file:
...
The older prototype files can be kept private or moved to an `examples/legacy/` directory if they are still useful:
...
- Replace hard-coded OAuth client values with clear documentation or environment variables where possible.
- Add `--dry-run` for commands that may download large files.
- Add safer default output permissions for token files.
- Avoid printing access tokens or AES keys unless `--show-sensitive` is explicitly passed.
- Add tests for `normalize_map_code`, `parse_size`, BPS manifest parsing, and URL construction.
- Add type checking / linting with `ruff` and `mypy` if the project grows.

## License

Recommended: **MIT License** if you want others to use, modify, and fork the code easily.

However, the MIT License only covers your Python code. It does not grant anyone rights to Epic Games, Fortnite, UEFN, island data, downloaded assets, AES keys, or any third-party content.

Create a `LICENSE` file with the standard MIT License text and your copyright line:

```text
Copyright (c) 2026 fn jp
```
